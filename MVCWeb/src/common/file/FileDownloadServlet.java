...
public class FileDownloadServlet extends HttpServlet {
	...
	{
		//1. 다운로드 받을 파일명 받기
		String fname = req.getParameter("fname"); // 실제로 업로드된 파일명(물리적 파일명)
		String ofname=req.getParameter("origin_fname");//원본 파일명
		System.out.println("fname="+fname);
		
		//2. 절대경로 만들기(application내장객체)
		ServletContext application = req.getServletContext();
		String upDir = application.getRealPath("/Upload");
		String path=upDir+File.separator+fname;
		System.out.println("path="+path);
		
		//res.setContentType("apllcation/unknown");
		res.setContentType("application/octet-stream");
		//알 수 없는 형식으로 지정 ==> 브라우저는 컨텐트 타입을 보고 표현할 수 있는 형식이면 해당 형식으로 표현하고
		//					잘 모르는 형식이면 다운로드창을 실행시켜 다운로드 받도록 처리한다.
		...
		//3. 해당 경로와 스트림 연결
		//(FileInputStream) => 파일을 읽어서 클라이언트 쪽으로 내보낸다.
